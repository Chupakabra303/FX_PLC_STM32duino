...
void loop() {
  while (Serial.available()) {
    ...
    if (ch == 'x') { // Переход в режим загрузчика при приеме по последовательному порту специального символа 'x'
      ...
      /* Переход в режим загрузчика с помощью символа инициализации бутлодера по UART. AN3155, USART bootloader code sequence.
       * Позволяет автоматически, без использования пина BOOT0 загружать скетч из Arduino IDE по нажатию кнопки Загрузка! */
      bootloaderInit(); // 
    }
  }  
